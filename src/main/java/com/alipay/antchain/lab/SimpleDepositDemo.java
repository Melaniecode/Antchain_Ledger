 ...
 * 基础存证示例
 */
public class SimpleDepositDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SimpleDepositDemo demo = new SimpleDepositDemo(new ChainConfig());
        ...
        try {
            ...
            String txHash = demo.deposit(message);

            // step 3: 链上查证
            System.out.println("Step 3, Querying message...");
            String onChainMessage = demo.query(txHash);
            System.out.printf(
                    "message on chain: %s, %s.%n",
                    onChainMessage,
                    message.equals(onChainMessage) ? "查证一致" : "查证不一致");
        } finally {
            // step 4: 关闭客户端
    ...
    public SimpleDepositDemo(ChainConfig config) {
        this.config = config;
        /* 账户地址 */
    ...
    public ClientEnv initMychainEnv() throws IOException {
        ...
        /* SSL配置 */
        InputStream skStream = SimpleDepositDemo.class.getClassLoader().getResourceAsStream(config.getTlsSecretKeyFilename());
        Objects.requireNonNull(skStream, "没找到TLS私钥文件: src/resources/" + config.getTlsSecretKeyFilename());
        InputStream certStream = SimpleDepositDemo.class.getClassLoader().getResourceAsStream(config.getTlsCertificateFilename());
        Objects.requireNonNull(certStream, "没找到TLS证书文件: src/resources/" + config.getTlsCertificateFilename());
        InputStream caStream = SimpleDepositDemo.class.getClassLoader().getResourceAsStream(config.getTlsCaFilename());
        ...
        InputStream accountSkStream = SimpleDepositDemo.class.getClassLoader().getResourceAsStream(config.getAccountSecretKeyFilename());
        ...
        env.setLogger(AbstractLoggerFactory.getInstance(SimpleDepositDemo.class));
     ...
     * @return 交易哈希
     */
    private String deposit(String message) {
        ...
        DepositDataRequest request = new DepositDataRequest(
                userIdentity,                               // 交易发起者
                userIdentity,                               // 交易接受者
                message.getBytes(StandardCharsets.UTF_8),   // 存证内容
     ...
     * @param txHash 交易哈希
     */
    private String query(String txHash) {
        ...
        byte[] data = queryTransactionResponse.getTransaction().getData();
